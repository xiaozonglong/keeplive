...
QString GPIOUtils::wpa_cli_scan(QString card)
{
    QString ret;
#ifdef MACRO_linux_arm
    {
        QString process =  "./install/wpa_cli";
        if( FileSys::isFileExist(process))
        {

            QStringList args;
            args<<"-i"<<card<<"scan";
            ret = utility::process(process,args);
        }
    }

#else
    qDebug()<<"wpa_cli_scan()"<<card;
#endif
    return ret;
}
QString GPIOUtils::wpa_cli_scan_result(QString card)
{
    QString ret;
#ifdef MACRO_linux_arm
    {
...
QString GPIOUtils::wpa_passphrase(QString apppath,QString account, QString passwd)
{
    QString ret;
...
#ifdef MACRO_linux_arm
    {
        ...
        if(FileSys::isFileExist(apppath))
        {

            QStringList args;
            args<<QString("\"%1\" \"%2\"").arg(account).arg(passwd)

                  ;
            //            .arg("/etc/wpa_supplicant.conf")
            //                qDebug()<<process<<args;
            ret = utility::process(apppath,args);
            qDebug()<<apppath<<args;
        }
    }

#else
    qDebug()<<"wpa_passphrase()"<<account<<passwd;
#endif
    return ret;
}
bool GPIOUtils::isExist(QString iofile)
{
    bool ret = false;
#ifdef MACRO_linux_arm
    QString process =  iofile;
    ret = FileSys::isFileExist(process);
    if( ret )
    {

    }
#else
    //    qDebug()<<"isExist()"<<iofile;
#endif
    return ret;
}

void GPIOUtils::userLedTrun(QString gpiostr, bool sw)
{
#ifdef MACRO_linux_arm
    QString process =  "./install/gpio_test";
    if( FileSys::isFileExist(process))
    {
        QStringList args;
        args<<gpiostr<<"output"<<QString::number(sw);
        //                qDebug()<<process<<args;
        utility::process(process,args);
    }
#else
    //    qDebug()<<"userLedTrun()"<<gpiostr<<sw;
#endif
}
void GPIOUtils::watchdogTrun(int time)
{
#ifdef MACRO_linux_arm
    QString process =  "./install/watchdog";
    if( FileSys::isFileExist(process))
    {
        QStringList args;
        args<<QString::number(time);
        //                qDebug()<<process<<args;
        utility::process(process,args);
    }
#else
    //    qDebug()<<"watchdogTrun()"<<time;
#endif
}
